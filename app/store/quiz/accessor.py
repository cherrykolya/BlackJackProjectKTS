...
class QuizAccessor(BaseAccessor):
    async def create_theme(self, title: str) -> Theme:
        #URL = f'postgresql://{self.app.config.database.user}:{self.app.config.database.password}@{self.app.config.database.host}:{self.app.config.database.port}/{self.app.config.database.database}'
        
        theme = Theme(id=len(await self.list_themes())+1, title=title)
        await ThemeModel.create(id=theme.id, title=theme.title)
        return theme
